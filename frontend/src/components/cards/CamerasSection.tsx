...
export function CamerasSection() {
  ...
  useEffect(() => {
    const i = setInterval(() => setTick(Date.now()), 8000)
  ...
  return (
    <div style={{ marginBottom: 'var(--space-xl)' }}>
      <div className="text-caption" style={{ marginBottom: 10 }}>Sicurezza</div>
      <div className="grid-fluid-lg">
        {cams.map((c) => (
          <button key={c.entity_id} onClick={() => setFull(c)}
            ...
            <CameraImg entityId={c.entity_id} tick={tick} />
...
function CameraImg({ entityId, tick }: { entityId: string; tick: number }) {
  ...
  return <img src={snapUrl(entityId, tick)} alt="" onError={() => setErr(true)} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
}

function CameraModal({ entity, onClose }: { entity: HassEntity; onClose: () => void }) {
  const [tick, setTick] = useState(Date.now())
  useEffect(() => {
    const i = setInterval(() => setTick(Date.now()), 2000) // refresh più veloce a schermo
    return () => clearInterval(i)
  }, [])
  ...
  return (
    <motion.div
      ...
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, zIndex: 3400, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 'var(--space-lg)', background: 'rgba(3,8,16,0.9)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)' }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, alignSelf: 'stretch', maxWidth: 900, margin: '0 auto', width: '100%' }}>
        <Video size={18} color="var(--accent)" />
        <span style={{ flex: 1, color: 'white', fontSize: 17, fontWeight: 700 }}>{camName(entity)}</span>
        <button onClick={onClose} aria-label="Chiudi" style={{ width: 36, height: 36, borderRadius: 11, cursor: 'pointer', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <X size={18} />
        </button>
      </div>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 900, borderRadius: 'var(--radius-lg)', overflow: 'hidden', background: '#0a1622', aspectRatio: '16/9' }}>
        <CameraImg entityId={entity.entity_id} tick={tick} />
      </div>
    </motion.div>
  )
}
